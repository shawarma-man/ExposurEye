...
whitelist = ["python", "apache netbeans", "openssl" ,"power", "postman", "gom", "opera", "mongodb", "github", "HI-TECH"]
...
system_cpes = ["cpe:2.3:o:microsoft:windows_10:{}:*:*:*:*:*:*:*","cpe:2.3:o:microsoft:windows:-:*:*:*:*:*:*:*","cpe:2.3:o:microsoft:windows_10:*:*:*:*:*:*:*:*"]
...
def getAppsCPE():
    ...
    for app in apps_data:
        ...
        cpe_str,score = get_best_cpe(app.get("ProgramName").replace(" ","_"), app.get("Vendor").replace(" ","_"), version)
        if score < 0.6:
            app["cpe"] = "N/A"
            ...
            app["cpe"] = cpe_str
            print(custom_prompt('information', f"Found appropriate match for: {app.get('ProgramName')}\n"))
...
def getAppsCVE():
    ...
    with open('temp.txt', 'w') as file:
        subprocess.run(['powershell', '-File', 'Get-RemoteProgram.ps1'], stdout=file)
    ...
    for index, line in enumerate(lines):
        ...
        if prop == "Version":
            ...
            if match:
                ...
                program["ProgramName"] = re.sub('\s+', ' ', program.get("ProgramName").replace(tmp, "")).strip()
        ...
        if prop == "SystemComponent":
            if not program.get("SystemComponent") and program.get("Vendor").lower() != "microsoft" and "Windows Driver Package" not in program.get("ProgramName"):
                ...
                if(program.get("Vendor") in program.get("ProgramName") and program.get("Vendor") != program.get("ProgramName") and program.get("Vendor").lower() not in whitelist):
                    program["ProgramName"] = (program.get("ProgramName").replace(program.get("Vendor"), "")).strip()

                if program.get("Version").replace(".", "") in program.get("ProgramName"):
                    program["ProgramName"] = (program.get("ProgramName").replace(program.get("Version").replace(".", ""), "")).strip()
    ...
    print(custom_prompt('information', "Searching for suitable CPE strings for installed applications...\n"))
    getAppsCPE()

    print(custom_prompt('information', "Searching for CVEs for installed applications...\n"))
    append_cve_details()
