...
def search_cpe_CVEs(cpe_string,version,cpe_index):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        c.execute("SELECT * FROM NVD_CVE_MATCH WHERE URI = ? OR URI = ? OR URI = ? OR URI = ?", (cpe_string,with_edition_no_version, no_edition_with_version, no_edition_no_version))
        ...
        for cve_match_item in cve_match_items: #cve_match_item == CVE_MATCH ELEMENT
            ...
            if Vulnerable == 1:
                cve_match_flag = True
                for i in range(4,8):
                    cve_match_flag = cve_match_flag and compare_versions(version,cve_match_item[i],i%2==0,i>5)
                if cve_match_flag or (cpe_string == cve_match_item[3]):
                    
                    c.execute(same_parent_query,(cve_match_item[1],cve_match_item[1])) #GET ALL CHILDREN WITH THE SAME PARENT OF CVE_MATCH_ITEM
                    cve_configurations_same_parent = c.fetchall()
                    
                    if len(cve_configurations_same_parent) == 0: #NO CHILDREN WITH THE SAME PARENT
                        c.execute(cve_details_query_ConfID,(cve_match_item[1],))
                        cve_details_items = c.fetchone()
                    ...
                    for cve_configuration_same_parent_item in cve_configurations_same_parent: 
                        c.execute("SELECT * FROM NVD_CVE_MATCH WHERE NVD_CVE_CONFIGURATION_ID = ?", (cve_configuration_same_parent_item[0],))
                        ...
                        for same_parent_cve_match in same_parent_cve_matches:
                            ...
                            if cpe_index.get(vendor_product_part):
                                app_version = cpe_index.get(vendor_product_part).get("Version")
                                app_cpe = cpe_index.get(vendor_product_part).get("cpe")
                                if compare_cpe_strings(app_cpe,same_parent_cve_match[3]):
                                    or_flag = True
                                    for i in range(4,8):
                                        or_flag = or_flag and compare_versions(app_version,same_parent_cve_match[i],i%2==0,i>5)
                                    if or_flag:
                                        break
...
# Create index on cpe field
    ...
    cve_details = {}
    for app in applications:
        print(custom_prompt("information", f"Searching for CVEs for: {app.get('ProgramName')}"))
